...
from dbx2.py2py3 import urlparse_wrapper as urlparse, urlquote
...
CONNECTIONS_ALLOWED_LIST_3_0 = ["serviceClass", "customizedJdbcUrl", "jdbcUseSSL", "jdbcDriverClass", "testQuery",
...
CONNECTION_TYPES_ALLOWED_LIST_3_0 = ["serviceClass", "supportedVersions", "jdbcUrlFormat", "jdbcUrlSSLFormat", "jdbcUseSSL",
...
INPUTS_SERVER_ALLOWED_LIST_3_0 = ["config_file", "keystore_password", "interval", "disabled"]

DB_INPUTS_ALLOWED_LIST_3_0 = ["description", "interval", "index", "source", "sourcetype", "host", "mode", "connection",
...
DB_OUTPUTS_ALLOWED_LIST_3_0 = ["description", "interval", "connection", "table_name", "using_upsert", "unique_key",
...
DB_LOOKUPS_ALLOWED_LIST_3_0 = ["description", "lookupSQL", "connection", "input_fields", "output_fields", "ui_query_mode",
...
exclude_list = ["antlr4-annotations-4.4.jar", "avro-compiler-1.7.6.jar", "dbx2.jar", "jetty-http-9.2.4.v20141103.jar",
...
def check_drivers_to_be_migrated():
    try:
        ...
        def migrate():
            ...
            shutil.copytree(old_dir, new_dir, symlinks=True, ignore=shutil.ignore_patterns(*exclude_list))
...
def _update_stanza_acl(stanza, acl):
    try:
        ...
        stanza.post("acl", body=urllib.parse.urlencode(prop))
    except:
...
def _filter_content(properties, allowed_list):
    return dict((k, v) for k, v in properties.items() if k in allowed_list)
...
def check_inputs_conf_mi_input(service, service_with_dbx_ns):
    if "inputs" in service.confs:
        all_inputs = [s for s in service.confs["inputs"] if s.name.startswith("mi_input://")]
        all_inputs = [s for s in all_inputs if s.access['app'] == service.namespace.app]
    else:
        all_inputs = []
    ...
    for an_input in all_inputs:
        ...
        _add_create_stanza_action(service, "db_inputs", name, _filter_content(properties, DB_INPUTS_ALLOWED_LIST_3_0), an_input.access)
        _add_remove_stanza_action("inputs.conf", an_input)


def check_inputs_conf_mi_output(service):
    if "inputs" in service.confs:
        all_outputs = [s for s in service.confs["inputs"] if s.name.startswith("mi_output://")]
        all_outputs = [s for s in all_outputs if s.access['app'] == service.namespace.app]
    else:
        all_outputs = []
    ...
    for an_output in all_outputs:
        ...
        _add_create_stanza_action(service, "db_outputs", name, _filter_content(properties, DB_OUTPUTS_ALLOWED_LIST_3_0), an_output.access)
        _add_remove_stanza_action("inputs.conf", an_output)


def check_inputs_conf_mi_lookup(service):
    if "inputs" in service.confs:
        all_lookups = [s for s in service.confs["inputs"] if s.name.startswith("mi_lookup://")]
        all_lookups = [s for s in all_lookups if s.access['app'] == service.namespace.app]
    else:
        all_lookups = []
    ...
    for a_lookup in all_lookups:
        ...
        _add_create_stanza_action(service, "db_lookups", name, _filter_content(properties, DB_LOOKUPS_ALLOWED_LIST_3_0), a_lookup.access)
        _add_remove_stanza_action("inputs.conf", a_lookup)


def check_inputs_conf_mi_session(service):
    all_sessions = [s for s in service.confs["inputs"] if s.name.startswith("mi_session://")]
    all_sessions = [s for s in all_sessions if s.access['app'] == service.namespace.app]
