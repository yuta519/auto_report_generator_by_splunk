...
class LogLevelHandler(SimpleRest):
    ...
    server_level_args = ["dbinput", "dboutput", "connector", "processor"]
    ...
    # DBX-4905: Adding a new processor loglevel
    server_module_info = {
        "dbinput": {
        ...
        "processor": {
            "property": "PROCESSOR_LOG_LEVEL",
            "regex": r'PROCESSOR_LOG_LEVEL=\w+',
            "packages": ["com.splunk.dbx.server.dbinput.task.processors"]
        },
        "dbxquery": {
            "property": "DBX_COMMAND_LOG_LEVEL",
            "regex": r'DBX_COMMAND_LOG_LEVEL=\w+'
        }
    }
    ...
    def _update_command_log_level(self, module, log_level, splunk_service):
        ...
        if module == "dbxquery":
            self._update_dbxquery_log_level_in_file(log_level)
            return
        ...
        log_args = next(iter(to_update.keys()))
    ...
    def _update_dbxquery_log_level_in_file(self, dbxquery_log_level):
        logger.debug("action=try_to_update_dbxquery_log_level %s" % str(dbxquery_log_level))
        try:
            jvmopts = jvm_options.read_query_server_vmopts()
            logger.debug("action=read_vmopts_from_file vmopts=%s" % jvmopts)
            property = self.server_module_info["dbxquery"]["property"]
            regex = self.server_module_info["dbxquery"]["regex"]
            jvmopts = jvm_options.set_property(jvmopts, property, regex, dbxquery_log_level)
